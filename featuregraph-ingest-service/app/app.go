...
func SetupRouter(router *gin.Engine) {
	...
	// do business
	router.POST("/events", reststats.HandleEndpointWithStats(handlePostEvents))
	router.GET("/appconfig/:acc/:aid", reststats.HandleEndpointWithStats(handleGetAppConfig))
...
func cacheResponse(c *gin.Context, maxAge time.Duration) {
	header := c.Writer.Header()
	header.Set("Cache-Control", fmt.Sprintf("public, max-age=%.f", maxAge.Seconds()))
}
